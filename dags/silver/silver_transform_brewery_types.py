...
def transform_fn(df: pd.DataFrame) -> pd.DataFrame:
    """
    Gera o dataframe silver_brewery_types a partir do dataframe de input.
    
    Parâmetros:
    -----------
    df : pd.DataFrame
        DataFrame original contendo a coluna brewery_type.
    
    Retorna:
    --------
    silver_brewery_types : pd.DataFrame
        DataFrame com colunas:
         - brewery_type (chave natural)
         - description (descrição amigável gerada a partir do brewery_type)
    """
    # Seleciona e deduplica os tipos de cervejaria
    silver = (
        df[['brewery_type']]
        .drop_duplicates()
    ...
    silver['description'] = (
        silver['brewery_type']
              .fillna('')
              .apply(lambda x: x.replace('_', ' ').title() if x else '')
    )
